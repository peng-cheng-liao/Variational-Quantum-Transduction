...
def plot_all_etas_SP(fs=25, mode_top="S", mode_bottom="P"):
    eta_list = [0.15, 0.20, 0.25, 0.30, 0.35, 0.40]
    eta_strs = [f"{eta:.2f}" for eta in eta_list]

    results_S = [load_best_for_eta(es, mode=mode_top) for es in eta_strs]
    results_P = [load_best_for_eta(es, mode=mode_bottom) for es in eta_strs]

    # shared limits across ALL subplots (both modes)
    max_len_global = max(
        max(np.linalg.norm(res["g1"]), np.linalg.norm(res["g2"]))
        for res in (results_S + results_P)
    )
    L = 1.75 * max_len_global if max_len_global > 0 else 1.0

    fig, axes = plt.subplots(4, 3, figsize=(15, 17), constrained_layout=True)

    def draw_panel(ax, res):
        ...
        ax.set_title(rf"$\eta={eta_str}, {mode} $", fontsize=fs)

        info_top = (
            rf"$\mathrm{{fidelity}}={fid:.2f}$" "\n"
            rf"$d={d_max}$" "\n"
            rf"$\angle(v,u)={theta12:.1f}^\circ$"
        )
        ax.text(
            0.03, 0.97, info_top,
    ...
    # First two rows: mode S (6 panels)
    for i, res in enumerate(results_S):
        r = i // 3       # 0,1
        c = i % 3
        draw_panel(axes[r, c], res)

    # Remaining two rows: mode P (6 panels)
    for i, res in enumerate(results_P):
        r = 2 + (i // 3)  # 2,3
        c = i % 3
        draw_panel(axes[r, c], res)

    plt.savefig(parent_dir + f"/Figs/GKP_state_match_mode=S_P.pdf", dpi=500)
    # plt.show()


if __name__ == "__main__":
    plot_all_etas_SP(fs=25, mode_top="S", mode_bottom="P")