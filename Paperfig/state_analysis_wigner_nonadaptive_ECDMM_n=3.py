...
def load_rhos(eta):
    state_RS = np.load(parent_dir + f"/Data_HPC/82/eta=" + eta + f"_depth=20_Nt=30/state_RS_best_feasible.npy")
    state_RS = qt.Qobj(state_RS, dims=[[Nt, Nt], [1, 1]])
    rho_S = qt.ptrace(state_RS, 1)

    state_PA = np.load(parent_dir + f"/Data_HPC/82/eta=" + eta + f"_depth=20_Nt=30/state_PA_best_feasible.npy")
...
plt.savefig(parent_dir + "/Figs/wigner_function_nonadaptive_ECD_MM_n=3.pdf", dpi=500)
plt.show()
