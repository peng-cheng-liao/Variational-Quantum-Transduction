...
statetype = "_CoherentInfo_EATele_fixedinput"
Nt = 30
...
np_constraint = 2
n_coefficient = 10
filepath = "Data/Transduction" + statetype + f"Nt={Nt}_depth={depth}_eta={eta}_ns={ns_constraint}_np={np_constraint}_N={N}_randomization={randomization}_"
...
def target(x):
    CI, ns_input, np_input, state_RS, state_PA = transduction_protocol_CoherentInfo_ECD_MM_EATele_fixedinput(theta, x, depth, Nt)
    loss = (-CI +
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    x = np.concatenate((alphas, betas, np.random.rand(6)))
    ...
    for nitern in range(N):
        ...
        if (np.argmax(ci_list) == nitern - 1 and
                ns_list[nitern] <= ns_constraint + 0.01 and np_list[nitern] <= np_constraint + 0.01):
