...
statetype = "_CoherentInfo_EA_test2_"
Nt = 30
...
def target(x):
    CI, ns_input, np_input, state_RS, state_PA = transduction_protocol_CoherentInfo_ECD_MM_EA_test2(np_constraint, eta, x, depth, Nt)
    loss = (-CI +
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    alphas = np.random.randn(12 * depth) * std_dev
    betas = np.random.uniform(-np.pi, np.pi, 12 * depth)
