...
def target(x):
    ...
    ef, np_input, state_in_AP, l0,l1, encoding_fidelity = transduction_protocol_ECD_MM_TE_EF(parameters_signals, theta, parameters, depth, Nt)
    fidelity = ef
    return -fidelity + penalty_energy(np_input, n_constraint, n_coefficient) + 100 * encoding_fidelity, ef, np_input, state_in_AP, l0,l1, encoding_fidelity
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    for nitern in range(N):
        loss, ef, np_input, state_in_AP, l0,l1, encoding_fidelity = target(x)
        loss.backward()
        ...
        inputstatelist.append(state_in_AP.detach().numpy())
