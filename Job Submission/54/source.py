...
statetype = "_CoherentInfo_EA_fixedinput_"
Nt = 30
d = 3
eta = 0.1
depth = 20
...
np_constraint = 2
n_coefficient = 10
filepath = f"Data/Transduction" + statetype + f"d={d}_Nt={Nt}_depth={depth}_eta={eta}_ns={ns_constraint}_np={np_constraint}_N={N}_randomization={randomization}_"
...
def target(x):
    CI, ns_input, np_input, state_RS, state_PA = transduction_protocol_CoherentInfo_ECD_MM_EA_fixedinput(d,eta, x, depth, Nt)
    loss = (-CI +
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    x = np.concatenate((alphas, betas, np.random.rand(9*d+2)))
