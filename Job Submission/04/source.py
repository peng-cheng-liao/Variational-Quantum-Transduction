...
rho_00 = cat0 @ dagger(cat0)
rho_01 = cat0 @ dagger(cat1)
rho_10 = cat1 @ dagger(cat0)
rho_11 = cat1 @ dagger(cat1)
...
def target(x):
    rho_out00, n1_input0, state_in0 = transduction_protocol_ECD_QMM_mixed(rho_00, theta, x, depth, Nt)
    rho_out01, n1_input1, state_in1 = transduction_protocol_ECD_QMM_mixed(rho_01, theta, x, depth, Nt)
    rho_out10, n1_input0, state_in0 = transduction_protocol_ECD_QMM_mixed(rho_10, theta, x, depth, Nt)
    rho_out11, n1_input1, state_in1 = transduction_protocol_ECD_QMM_mixed(rho_11, theta, x, depth, Nt)

    f00 = dagger(cat0) @ rho_out00 @ cat0
    f01 = dagger(cat0) @ rho_out01 @ cat1
    f10 = dagger(cat1) @ rho_out10 @ cat0
    f11 = dagger(cat1) @ rho_out11 @ cat1


    fidelity = torch.real((f00+f01+f10+f11)/4)
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    for nitern in range(N):
        ...
        if (nitern % save_frequency == 0 or nitern == N - 1) and save:
            filepath = f"Data/Transduction" + statetype + f"Nt={Nt}_depth={depth}_energy={n_constraint}_eta={np.around(eta,2)}_N={N}_randomization={randomization}_"
