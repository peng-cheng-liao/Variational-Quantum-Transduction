...
def target(x):
    CI, ns_input, state_RS= transduction_protocol_CoherentInfo_Tele(np_constraint, eta, x, depth, Nt)
    loss = (-CI +
            penalty_energy(ns_input, ns_constraint, n_coefficient))
    return loss, CI, ns_input, state_RS


def minimize_target(N=N, disp=disp, save=save):
    global state_RS_save, x_save
    time1 = datetime.now()
    np.random.seed()
    ...
    for nitern in range(N):
        loss, CI, ns_input, state_RS = target(x)
        loss.backward()
