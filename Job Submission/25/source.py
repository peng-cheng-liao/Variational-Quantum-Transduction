...
def target(x):
    ...
    parameters = x[4*depth:12*depth]
    ef, np_input,ns,state_in_P, l0,l1, encoding_fidelity = transduction_protocol_ECD_M_TE_EF(parameters_signals, theta, parameters, depth, Nt)
    fidelity = ef
    loss = (-fidelity + penalty_energy(np_input, n_constraint, n_coefficient) +
            penalty_energy(ns, n_constraint, n_coefficient) + 100 * encoding_fidelity)
