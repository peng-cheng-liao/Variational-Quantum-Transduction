...
Nt = 30
ETA_FIXED = 0.30
eta_env = os.environ.get("ETA")
if eta_env is not None and not np.isclose(float(eta_env), ETA_FIXED):
    raise ValueError(f"Job 96 fixes eta={ETA_FIXED:.2f}; got ETA={eta_env}")
eta = ETA_FIXED

DEPTH_LIST = (2, 4, 6, 8, 10, 12, 14, 16, 18)
depth = int(os.environ.get("DEPTH", str(DEPTH_LIST[0])))
if depth not in DEPTH_LIST:
    raise ValueError(f"DEPTH must be one of {DEPTH_LIST}; got {depth}")
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Job 96 depth-convergence training for eta=0.30, ns=np=2."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main():
    ...
    if eta_tag is None:
        eta_tag = f"{eta:.2f}"
    depth_tag = os.environ.get("DEPTH_TAG")
    if depth_tag is None:
        depth_tag = f"{depth:02d}"

    run_dir = os.environ.get("RUN_DIR",f"runs/eta={eta_tag}_depth={depth_tag}_Nt={Nt}/seed_{seed}")
    # -----------------------
    # Top-level training_parameters directory
    # -----------------------
    ckpt_dir = os.environ.get("CKPT_DIR",f"training_parameters/eta={eta_tag}_depth={depth_tag}_Nt={Nt}/seed_{seed}")
    ...
    expected_parameter_len = 24 * depth
    if args.dry_run:
        print("[DRY_RUN] Job 96 configuration")
        print(f"  eta                   = {eta:.2f}")
        print(f"  eta_tag               = {eta_tag}")
        print(f"  depth                 = {depth}")
        print(f"  depth_tag             = {depth_tag}")
        print(f"  Nt                    = {Nt}")
        print(f"  seed                  = {seed}")
        print(f"  max_steps             = {max_steps}")
        print(f"  ns_constraint         = {ns_constraint}")
        print(f"  np_constraint         = {np_constraint}")
        print(f"  expected len(x)       = {expected_parameter_len}")
    ...
    if resume and os.path.exists(resume_path):
        ...
        if len(x) != expected_parameter_len:
            raise RuntimeError(
                f"Internal parameter length mismatch: len(x)={len(x)} "
                f"expected={expected_parameter_len}"
            )
