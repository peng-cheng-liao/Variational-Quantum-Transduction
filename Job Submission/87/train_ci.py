...
#state_P = np.load(f"Input_state/Nonadaptive_VQT_EA_ns=np=3_eta={eta}_state_P.npy")
#state_initial_RS = torch.tensor(state_RS)
#state_initial_PA = torch.tensor(state_P)
...
def target(x, apply_penalty: bool):
    CI, ns_input, np_input, state_RS, state_P = transduction_protocol_CoherentInfo_ECD_M_EA(eta, x, depth, Nt)
...
def main():
    ...
    rs_best_feas_path = os.path.join(run_dir, "state_RS_best_feasible.npy")
    pa_best_feas_path = os.path.join(run_dir, "state_P_best_feasible.npy")
    ...
    if resume and os.path.exists(resume_path):
        ...
        alphas = rng.standard_normal(8 * depth) * std_dev_alphas
        betas = rng.uniform(-np.pi, np.pi, 8 * depth)
    ...
    for nitern in range(start_iter, max_steps):
        ...
        loss, CI, ns_input, np_input, state_RS, state_P = target(x, apply_penalty)
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        ...
        if is_feasible and ci_val > best_feas_ci:
            ...
            np.save(pa_best_feas_path, state_P.detach().numpy())
