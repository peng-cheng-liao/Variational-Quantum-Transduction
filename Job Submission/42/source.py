...
statetype = "_CoherentInfo_MM_EA_TMSInitial_"
Nt = 30
...
def target(x):
    CI, ns_input, np_input, state_RS, state_PA = transduction_protocol_CoherentInfo_ECD_MM_EA_TMSInitial(eta, x, depth, Nt)
    loss = (-CI +
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    alphas = np.random.randn(12 * depth) * 0
    betas = np.array([np.pi/2,np.pi]* 6 * depth)
