...
statetype= "_cat_code_states_weight=0011_"
Nt = 20
state_signals = generate_cat_code(1,Nt)
cat0 = state_signals[0]
cat1 = state_signals[1]
catp = (cat0+cat1)/np.sqrt(2)
catm = (cat0+cat1)/np.sqrt(2)
eta = 0.1
...
def target(x):
    rho_out0, n1_input0, state_in0 = transduction_protocol_ECD_MM(cat0, theta, x, depth, Nt)
    rho_out1, n1_input1, state_in1 = transduction_protocol_ECD_MM(cat1, theta, x, depth, Nt)
    rho_out2, n1_input2, state_in2 = transduction_protocol_ECD_MM(catp, theta, x, depth, Nt)
    rho_out3, n1_input3, state_in3 = transduction_protocol_ECD_MM(catm, theta, x, depth, Nt)
    f0 = state_fidelity(rho_out0, cat0)
    f1 = state_fidelity(rho_out1, cat1)
    f2 = state_fidelity(rho_out2, catp)
    f3 = state_fidelity(rho_out3, catm)
    fidelity = f0*weight[0]+f1*weight[1]+f2*weight[2]+f3*weight[3]
    #fidelity = min(f0,f1,f2,f3)
    return -fidelity + penalty(n1_input0, n_constraint, n_coefficient), n1_input0, state_in0, f0, f1, f2, f3
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    for nitern in range(N):
        loss, inputenergy, inputstate,f0,f1,f2,f3 = target(x)
        loss.backward()
        ...
        f2list.append(f2.detach().numpy())
        f3list.append(f3.detach().numpy())
        ...
        if (nitern % save_frequency == 0 or nitern == N - 1) and save:
            ...
            np.save(filepath + "f2", f2list)
            np.save(filepath + "f3", f3list)
