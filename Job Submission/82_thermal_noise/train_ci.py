...
# -----------------------
statetype = "_CoherentInfo_EA_thermal_"
Nt = int(os.environ.get("NT", "30"))
eta = float(os.environ.get("ETA", "0.25"))
depth = int(os.environ.get("DEPTH", "20"))

# Mode 2: noise-aware retraining.
# The objective below trains directly through the noisy transducer. A separate
# fixed-protocol robustness scan can load Job 82 parameters and call the same
# protocol with those fixed parameters.
kappa_o = float(os.environ.get("KAPPA_O", "1.0"))
n_o = float(os.environ.get("N_O", "0.0"))
kappa_m = float(os.environ.get("KAPPA_M", "1.0"))
n_m = float(os.environ.get("N_M", "0.0"))

env_cutoff_o_env = os.environ.get("ENV_CUTOFF_O")
env_cutoff_m_env = os.environ.get("ENV_CUTOFF_M")
env_cutoff_o = int(env_cutoff_o_env) if env_cutoff_o_env else None
env_cutoff_m = int(env_cutoff_m_env) if env_cutoff_m_env else None
...
ns_constraint = float(os.environ.get("NS_CONSTRAINT", "2"))
np_constraint = float(os.environ.get("NP_CONSTRAINT", "2"))
n_coefficient = 10.0
...
def main():
    ...
    output_tag = (
        f"ECD_MM_EA_thermal_ns{_tag_float(ns_constraint, 0)}"
        f"_np{_tag_float(np_constraint, 0)}"
        f"_eta{_tag_float(eta)}"
        f"_ko{_tag_float(kappa_o)}"
        f"_no{_tag_float(n_o)}"
        f"_km{_tag_float(kappa_m)}"
        f"_nm{_tag_float(n_m)}"
        f"_cutoff{Nt}"
        f"_seed{seed}"
    )

    run_dir = os.environ.get("RUN_DIR", f"runs/{output_tag}")
    # -----------------------
    # Top-level training_parameters directory
    # -----------------------
    ckpt_dir = os.environ.get("CKPT_DIR", f"training_parameters/{output_tag}")
    ...
    # File paths
    # -----------------------
    filepath_prefix = os.path.join(run_dir, f"{output_tag}_")
    metadata = {
        "mode": "noise_aware_retraining",
        "eta": eta,
        "eta_tag": eta_tag,
        "depth": depth,
        "Nt": Nt,
    ...
    if resume and os.path.exists(resume_path):
        ...
        print(
            ...
            f"         run_dir={run_dir}\n"
            f"         ckpt_dir={ckpt_dir}\n"
            f"         thermal_noise: kappa_o={kappa_o} n_o={n_o} "
            f"kappa_m={kappa_m} n_m={n_m}\n"
