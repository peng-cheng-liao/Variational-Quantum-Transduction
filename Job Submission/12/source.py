...
def target(x):
    ...
    ef, np_input,ns,state_in_P, l0,l1, encoding_fidelity = transduction_protocol_ECD_MM_TE_EF(parameters_signals, theta, parameters, depth, Nt)
    fidelity = ef
    loss = (-fidelity
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    for nitern in range(N):
        loss, ef, np_input, f, state_in_P, l0,l1, encoding_fidelity= target(x)
        loss.backward()
