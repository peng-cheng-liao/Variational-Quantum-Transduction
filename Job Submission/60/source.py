...
def target(x):
    CI, ns_input, np_input, state_RS, state_P = transduction_protocol_CoherentInfo_GKP(eta, d1, d2, j2, x, Nt)
    loss = (-CI +
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    for nitern in range(N):
        ...
        if (np.argmax(ci_list) == nitern - 1 and ns_list[nitern] <= ns_constraint + 0.05 and np_list[nitern] <= np_constraint + 0.05 ):
            x_save = x.detach().numpy()
            state_RS_save = state_RS.detach().numpy()
            print("save this data")
            np.save(filepath + "state_RS", state_RS_save)
            np.save(filepath + "state_P", state_P)
