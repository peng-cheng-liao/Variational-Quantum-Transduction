...
def target(x):
    CI, ns_input, np_input, state_RS, state_PA = transduction_protocol_CoherentInfo_ECD_MM_EATele_fixedinput(theta, ns_constraint,x, depth, Nt)
    loss = (-CI +
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    x = np.concatenate((alphas, betas, np.random.rand(5)))
