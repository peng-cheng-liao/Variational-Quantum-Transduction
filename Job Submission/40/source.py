...
def target(x,y):
    ...
    :param x: parameters for encoding and decoding gates
    :param y: parameters for state preparation in  RS
    :return:
    """
    parameters = torch.cat((x[0*depth:8*depth], y[0*depth:4*depth], x[8*depth:16*depth],y[4*depth:8*depth]))
...
def minimize_target(N=N, disp=disp, save=save):
    ...
    scheduler_y = StepLR(opt_y, step_size=1000, gamma=0.5)
    for nitern in range(N):
        optimize_x = (nitern // block_size) % 2 == 0
        opt_x.zero_grad()
        opt_y.zero_grad()
