...
def state_fidelity(state1: torch.Tensor, state2: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    """
    Fidelity between two quantum states (pure vector Nx1 or density matrix NxN).

    - Pure/Pure: F = |<psi|phi>|^2
    - Pure/Mixed: F = <psi|rho|psi>
    - Mixed/Mixed (Uhlmann): F = (Tr sqrt( sqrt(rho) sigma sqrt(rho) ))^2

    This implementation avoids backprop through complex eigenvectors by using:
      rho = A A^† (Cholesky factor A),
      Tr sqrt( A^† sigma A ) = sum_i sqrt(lambda_i) using eigvalsh (eigenvalues only).
    """

    def is_pure(s: torch.Tensor) -> bool:
        return s.ndim == 2 and s.shape[1] == 1

    def is_mixed(s: torch.Tensor) -> bool:
        return s.ndim == 2 and s.shape[0] == s.shape[1] and s.shape[1] != 1

    def to_density(s: torch.Tensor) -> torch.Tensor:
        if is_pure(s):
            return s @ s.conj().T
        if is_mixed(s):
            return s
        raise ValueError("State must be Nx1 (pure) or NxN (density matrix).")

    def hermitize(A: torch.Tensor) -> torch.Tensor:
        return (A + A.conj().T) * 0.5

    def safe_cholesky_psd(rho: torch.Tensor, eps_: float) -> torch.Tensor:
        """
        Return A such that rho_pd = A A^†, with rho_pd made numerically PSD/PD.
        Uses cholesky_ex; if needed, shifts by (-min_eig + eps).
        """
        rho = hermitize(rho)

        # First try Cholesky directly
        A, info = torch.linalg.cholesky_ex(rho)
        if info.item() == 0:
            return A

        # If not PD, shift spectrum upward using eigenvalues only (no eigenvectors).
        evals = torch.linalg.eigvalsh(rho).real
        min_eig = evals.min()
        shift = torch.clamp(-min_eig + eps_, min=0.0)

        I = torch.eye(rho.shape[0], device=rho.device, dtype=rho.dtype)
        rho_pd = rho + shift * I

        A2, info2 = torch.linalg.cholesky_ex(rho_pd)
        if info2.item() != 0:
            # Last-resort extra jitter
            rho_pd = rho_pd + (10.0 * eps_) * I
            A2 = torch.linalg.cholesky(rho_pd)
        return A2

    # Promote to complex dtype for consistent behavior
    dtype = torch.complex128 if (state1.is_complex() or state2.is_complex()) else torch.complex128
    state1 = state1.to(dtype)
    state2 = state2.to(dtype)

    # Pure / Pure
    if is_pure(state1) and is_pure(state2):
        inner = (state1.conj().T @ state2)[0, 0]
        return inner.abs() ** 2

    rho = to_density(state1).to(dtype)
    sigma = to_density(state2).to(dtype)
    rho = hermitize(rho)
    sigma = hermitize(sigma)

    # Pure / Mixed shortcut: F = <psi|rho|psi>
    if is_pure(state1) and is_mixed(state2):
        psi = state1
        val = (psi.conj().T @ sigma @ psi)[0, 0]
        return val.real.clamp_min(0.0)

    if is_mixed(state1) and is_pure(state2):
        phi = state2
        val = (phi.conj().T @ rho @ phi)[0, 0]
        return val.real.clamp_min(0.0)

    # Mixed / Mixed:
    # Choose any factor A with rho ≈ A A^† (Cholesky of shifted rho)
    A = safe_cholesky_psd(rho, eps)

    # M = A^† sigma A is Hermitian PSD (up to numerics)
    M = hermitize(A.conj().T @ sigma @ A)

    # Tr sqrt(M) = sum_i sqrt(lambda_i), eigenvalues only
    lam = torch.linalg.eigvalsh(M).real
    lam = lam.clamp_min(0.0)
    tr_sqrt = torch.sqrt(lam).sum()

    fid = tr_sqrt ** 2
    return fid.real.clamp_min(0.0)
...
def position_eigenstate_coeffs_stable(x: float, N: int) -> np.ndarray:
    """
    Stable computation of coefficients <n|x> (harmonic oscillator position wavefunctions)
    using the recurrence for normalized Hermite functions:
    ...
    Returns:
        coeffs[n] = phi_n(x)  (real-valued for real x)
    """
    coeffs = np.zeros(N, dtype=np.complex128)

    # phi_0
    phi0 = (np.pi ** (-0.25)) * np.exp(-0.5 * x * x)
    coeffs[0] = phi0
    ...
    for n in range(1, N - 1):
        # computes phi_{n+1} from phi_n (curr) and phi_{n-1} (prev)
        a = math.sqrt(2.0 / (n + 1)) * x
        b = math.sqrt(n / (n + 1))
        nxt = a * curr - b * prev
        coeffs[n + 1] = nxt
        prev, curr = curr, nxt
